...
-- Namespace for this mod's script files. Each Noita mod runs in its own Lua
-- context, and every file dofile_once'd below executes in this mod's context:
-- globals cross file boundaries, locals don't. This table groups the mod's
-- cross-file functions instead of scattering loose globals. Submodules assign
-- into it directly (function SpellPush.foo(...) end) rather than ending with
-- "return {...}": dofile_once only runs a given path once per context, so a
-- hot-reload's second call is a documented no-op that returns nothing --
-- capturing a return value would get silently overwritten with nil, while
-- direct assignments just stay in place.
SpellPush = SpellPush or {}

-- Flip to true to trace drag detection/resolution on-screen and in logger.txt.
local DEBUG = false

function SpellPush.debug_log(message)
	if DEBUG then
		print("spell_push: " .. message)
		GamePrint("spell_push: " .. message)
	end
end

if not keycodes_loaded_ok then
	SpellPush.debug_log("keycodes.lua failed to load; using numeric fallback")
end

SpellPush.MIN_DRAG_DISTANCE_PIXELS = 3 -- ignore mouse jitter smaller than this when reading drag direction

dofile_once("mods/spell_push/files/scripts/snapshot.lua")
dofile_once("mods/spell_push/files/scripts/layout.lua")
dofile_once("mods/spell_push/files/scripts/drag.lua")
...
local function try_convert_swap_to_insert(player, baseline, swap)
	...
	if not baseline_wand then
		SpellPush.debug_log("skip: target wand not in baseline")
		return
	end

	local new_layout = SpellPush.compute_insert_layout(
		baseline_wand,
		swap.dragged_spell,
	...
	if not new_layout then
		SpellPush.debug_log("skip: wand has no free slot, vanilla swap stands")
		return
	end

	SpellPush.debug_log("applying insert layout")
	SpellPush.apply_insert_layout(player, swap.target_wand, new_layout)
	pending_correction = {
...
local function poll()
	local player = SpellPush.get_player()
	...
	if pending_correction then
		SpellPush.debug_log("re-asserting pending correction, frames_left=" .. pending_correction.frames_left)
		SpellPush.apply_insert_layout(player, pending_correction.wand, pending_correction.layout)
	...
	local current_snapshot = SpellPush.take_snapshot(player)

	if not drag_state then
		SpellPush.debug_log("inventory opened, baseline established")
		drag_state = fresh_drag_state(current_snapshot, alt_held_now)
		return
	end

	local moved, appeared, disappeared = SpellPush.diff_snapshots(drag_state.baseline, current_snapshot)
	...
	SpellPush.debug_log(string.format("diff: moved=%d appeared=%d disappeared=%d",
		#moved, #appeared, #disappeared))

	local release_x, release_y = InputGetMousePosOnScreen()
	local swap = SpellPush.classify_swap(drag_state.baseline, current_snapshot, moved, appeared, disappeared,
		last_press_x, last_press_y, release_x, release_y)

	if not swap then
		SpellPush.debug_log("could not classify as a swap; leaving vanilla result in place")
	else
		...
		-- "vanilla" (default): insert-and-push unless Alt is held. "spell_push": the reverse.
		local alt_held = alt_held_now or drag_state.alt_was_held_last_frame
		local hold_alt_to_activate = ModSettingGet("spell_push.left_alt_behaviour") == "spell_push"
		local should_insert = (alt_held == hold_alt_to_activate)

		if not should_insert then
			SpellPush.debug_log("mode says vanilla; leaving swap in place")
		else
			SpellPush.debug_log(string.format("converting swap: dragged=%s displaced=%s wand=%s target_slot=%s origin_slot=%s",
	...
	drag_state = fresh_drag_state(SpellPush.take_snapshot(player), alt_held_now)
end
